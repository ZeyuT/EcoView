...
int		  max_base_z, min_base_z, shifted_z, pre_z, thickness;
if (oct_stack == NULL)
...
for (x = 0; x < TotalSlices; x++)
{
	...
	{
		z = CutoffHeight;
		while (z < ROWS-1 && oct_x_proj[z*COLS + y] < 50 && oct_y_proj[z*TotalSlices + x] < 30)
			z++;
		baseline_cal[x*COLS + y] = z;
		if (max_base_z && z - max_base_z > 10) z = max_base_z;
		baseline_cal[x * COLS + y] = z;
...
{
	z = CutoffHeight;
	while (z < ROWS - 1 && oct_x_proj[z * COLS + y] < 50)
		z++;
...
{
	z = CutoffHeight;
	while (z < ROWS - 1 && oct_y_proj[z * TotalSlices + x] < 30)
		z++;
...
for (x=0; x<TotalSlices; x++)
  for (y=0; y<COLS; y++)
	{
	z = max(CutoffHeight, baseline_cal[x * COLS + y] - 50);
...
for (x=0; x<TotalSlices; x++)
  for (y=0; y<COLS; y++)
	...
	// ignore small noise (< 2 voxels) in thickness calculation
	if (thickness>1) {
		coverage_ratio += 1;
...
for (x=0; x<TotalSlices; x++)
  for (y=0; y<COLS; y++)
	{
		// ignore small noise (< 2 voxels) in thickness calculation
		if (abs(baseline_cal[x * COLS + y] - oct_depth_cal[x * COLS + y])<2)
